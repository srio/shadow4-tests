...
if __name__ == "__main__":
    ...
    # shadow3
    #
    from shadow4tests.oasys_workspaces.mirrors_branch3_mesh_perfect_ellipsoidal import define_source, run_source, define_beamline, run_beamline
    ...
    mirror1 = S4SurfaceDataMirrorElement(optical_element=S4SurfaceDataMirror(
                                            name=name,
                                            surface_data_file="../oasys_workspaces/mirror111.h5",
    ...
    plotxy(beam3, 1, 3, nbins=201, nolost=1, xrange=[-1e-6,1e-6], yrange=[-1e-6,1e-6], title="%s shadow3" % name)
    plotxy(beam4, 1, 3, nbins=201, nolost=1, xrange=[-1e-6,1e-6], yrange=[-1e-6,1e-6], title="%s shadow4" % name)
